...
namespace DB_Lab_4
{
    public partial class Form1 : Form
    {
        ...
        private void Fill(int tableIndex)
        {
            try
            {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // ([Товары].[группа] = 4) AND ([Товары].[наименование] = 'Помидор') AND ([Товары].[фассовка] = 20) AND ([Товары].[цена] = 35)

                Regex colReg = new Regex(@"[.][[][а-я]+[]]", RegexOptions.IgnoreCase);
                MatchCollection matchCol = colReg.Matches(textBox1.Text);
                foreach (Match match in matchCol)
                {
                    query += match.ToString().Trim(new char[] { '.', '[', ']' }) + ", ";
                }
                query = query.Remove(query.Length - 2, 2);
                query += ") values(";

                Regex regex = new Regex(@"['][а-я | /s]+[']|[0-9]+[.][0-9]+|[0-9]+", RegexOptions.IgnoreCase);
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                OleDbCommand command = new OleDbCommand();
                command.Connection = connection;
                string query = "UPDATE Группы INNER JOIN Товары ON Группы.код_группы = Товары.группа " +
                    "SET Группы.[группа товара] = [группа товара], Товары.цена = [цена]*0.9\n";
                if (textBox1.Text != "") query += "WHERE " + textBox1.Text + "\n";
                command.CommandText = query;
